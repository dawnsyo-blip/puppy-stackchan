...
- **碰屏幕 → "小开心"（摸头反应）**：`enter_xiaokaixin()`，不设状态限制、
  ...
  **`play_xiaokaixin_animation()` 曾经踩过"舵机噪音防误触发语音"一节记录
  ...
  的角度抹掉。表现是"小开心"一播完舵机就回正、不再朝着人，紧接着的手势扫描
...
- **触摸响应"特别慢"：根因是主循环轮询节奏被人脸检测的耗时拖垮，不是触摸
  ...
  **上面这套修复留了一个没堵上的口子，后来被实测暴露**（用户反馈：碰屏幕
  →小开心之间延迟依然很明显，实测 ~2s）：`check_touch()` 从轮流里摘出来了
  ...
  **这次改完后台线程，实机测试当场就暴露了一个新坑**：碰屏幕触发"小开心"
  时，`play_xiaokaixin_animation()` 连续发了好几个 `/servo` 请求，其中两个
  ...
  **这把锁本身又在下一轮反馈里暴露了一个新的权衡问题**：用户反馈"碰屏幕→
  小开心""双击→兴奋"这些触摸反应有时"还是好久"，报的数字（~5s）跟
