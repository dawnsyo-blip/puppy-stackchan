...
**Ctrl+C 退出后屏幕变黑是预期行为，不是真的关机**：`/display?off=1` 只是
`LGFXBase::sleep()`（背光+面板睡眠），ESP32、WiFi、HTTP server 全部继续
正常运行——想让屏幕重新亮起来，只需要重新跑一次 `python host/puppy_
engine_v4.py`（`run()` 开机第一步就会调 `set_display(on=True)`），不需要
碰物理电源键。**物理电源键按两下才能唤醒屏幕，是 AXP2101 芯片本身的硬件
行为，不是这份固件代码触发的**：`firmware.ino` 里已经没有任何代码读取
`M5.BtnPWR`（见下面撤销的那次尝试），按一下电源键如果按住够久（寄存器
配置的 `PowerOff=4sec` 阈值），会触发芯片真正切断电源（ESP32 完全失电，
跟软件层面的"屏幕睡眠"是两回事）；断电状态下再按一下会重新上电、走一次
完整冷启动，`setup()` 跑完屏幕自然重新显示内容——表现上就是"按两下"，
第一下是真关机、第二下是真开机，这条路径完全在固件代码控制范围之外，
没有可以修的地方。

**曾经尝试过让物理电源键长按也触发同一个视觉效果（纯固件实现），已经撤销
——留个教训记录**：CoreS3 的电源管理芯片是 AXP2101，`setup()` 里被
...
线**——这次第一次真正暴露问题已经是在用户的日常使用中。`/display` 这个
HTTP 接口本身、以及 host 端 Ctrl+C 退出时触发的 `play_shutdown_animation()`
（纯粹靠 host 主动发 HTTP 请求触发，不依赖设备自己判断有没有长按）完全没
问题，都原样保留，这次只撤销了"固件自己监听物理电源键"这一条路径——以后
如果想再尝试这个方向，务必先做纯观察验证。
