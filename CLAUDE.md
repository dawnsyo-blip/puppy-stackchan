...
- **字幕框跟按钮在屏幕上会重叠，这次一起改了固件**：字幕框原来 308px 宽
  ...
  起动。
- **音量参数以实测脚本为准，不是设计草案**：`ANIMALESE_VOLUME_BOOST` 用的
  是 `3.0`（1W 小喇叭偏小声，这个值是独立原型 `host/animalese_test.py`
  实机试出来的），不是最初设计文档里写的 `1.0`——迁移这个功能时如果看到
  两份材料对不上，以实际跑过、能听到声音的脚本参数为准。
