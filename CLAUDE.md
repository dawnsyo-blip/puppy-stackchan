...
**物理电源键长按也能触发同一个视觉效果，纯固件实现，不依赖 host 脚本在跑**：
CoreS3 的电源管理芯片是 AXP2101，`M5StackChan.begin()`（内部调
`M5.begin()`）初始化时被 M5Unified 配置成"长按 1 秒触发芯片内部的长按标记
（IRQ 状态位）/ 长按满 4 秒芯片自己硬件断电"（`setup()` 里 Axp2101 那段
寄存器初始化数组旁的原厂注释：`0x27, 0x00 // PowerKey Hold=1sec /
PowerOff=4sec`）——4 秒这个硬件断电阈值完全没动过，还是原来的安全兜底，不
依赖任何软件；这里只是"蹭"了芯片提前在 1 秒时给出的那个信号：`M5.update()`
会把它翻译成标准 `Button_Class` 接口 `M5.BtnPWR.wasHold()`（芯片那个 IRQ
状态位读一次就自动清零，天然是边沿触发，不需要自己另外做防抖/去重），
`loop()` 里检测到就调 `playShutdownAnimation()`——跟 host 端
`play_shutdown_animation()` 是同一个视觉设计（闭眼、灯灭、停 1 秒、关屏幕），
但纯固件实现：用户是直接按物理电源键关机，这时候电脑上的 host 脚本未必在
跑，不能依赖它发 HTTP 指令过来。真正断电还是要等满 4 秒，中间空出的 ~3 秒
窗口够这套动画播完还有富余。`delay(1000)` 会短暂卡住这一轮 `server.
handleClient()`，但设备最多几秒后就要断电，没必要再处理新请求；avatar 的
渲染任务在独立 FreeRTOS 任务里跑，不受这里阻塞影响，闭眼表情照常能画出来。
**这条路径完全没有改动或关闭芯片那个 4 秒硬件断电的保险**——如果哪天固件
卡死、`loop()` 完全没机会跑到这段代码，用户继续按住物理键，4 秒之后芯片
照样会自己断电，跟没有这个功能之前的兜底行为完全一样。
